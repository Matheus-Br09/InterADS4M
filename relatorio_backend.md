...
1. **Entregar as senhas novas e avisar sobre reuso:** a rotação em si está feita (item 24), mas as senhas novas **ainda não foram entregues** — elas só existem no terminal onde foram geradas, então se forem perdidas é `gestor:senha` / `apoiador:senha` de novo. Antes de entregar, rode **nesta ordem**: `php artisan migrate` e depois `php artisan apoiadores:exigir-troca --todos` (item 25). Sem a migration o comando falha com `Unknown column 'trocar_senha_obrigatorio'`, e o erro não tem nada a ver com senha — é a ordem. A conta fica presa na tela de troca até a pessoa criar senha própria, que é o que faz a senha de WhatsApp morrer no primeiro acesso. Confira o resultado com `php artisan apoiadores:listar` (coluna `troca`): as 3 contas de apoiador em `obrigatoria` e a de gestão em `nao` — se a gestão aparecer marcada, o `--todos` foi para o `email` errado. A entrega precisa ser por canal privado (e nada de print de tela: o terminal com a senha é exatamente o que não deve circular), e vale pedir que troquem a senha nos outros serviços onde usavam a mesma, porque senha reutilizada não é afetada por nada que tenhamos feito aqui. Quem não entrar nunca destrava sozinho: o `--desfazer` do mesmo comando é o caminho de volta.
...
5. **Cookies e HTTPS:** `SESSION_SECURE_COOKIE` já está documentado no `.env.example` (26/09) como `false`, com o aviso de que só pode virar `true` sob HTTPS — ligado em `http://localhost` ou em IP de rede local, o navegador não devolve o cookie e todo mundo fica deslogado sem mensagem. Segue faltando HSTS, cabeçalhos de segurança, `trustProxies` e a decisão de qual variável usar conforme o acesso (o projeto ainda não está em produção, então isso só se resolve no momento do deploy).
6. **Força bruta e senha fraca (parcialmente resolvido em 26/09):** ver item 20 do que já foi feito - a senha fraca e o bloqueio por conta entraram. A conta é bloqueada por 5 tentativas a cada 5 minutos para o par (e-mail, IP), e o limite por IP no login é de 10/min. Continua de pé o mesmo furo: um atacante distribuído ganha 5 tentativas por IP de cada vez.
